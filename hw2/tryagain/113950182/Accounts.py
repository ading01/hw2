...
class Account:
    ...
    def add_transaction(self, amt, date=None, exempt=False):
        ...
        if self.latest_date == None:
            self.latest_date = t.get_date()
        elif t.get_date() < self.latest_date:
        ...
        if t.is_exempt() or (balance_ok and limits_ok):
            ...
            self.last_exempt = t._exempt
            self.latest_date = t.get_date()
            logging.debug(f"Created transaction: {self._account_number}, {Decimal(amt)}")
    ...
    def _interest(self):
        ...
        self.add_transaction(self.get_balance() * self._interest_rate, date=str(new_date), exempt=True)
...
class CheckingAccount(Account):
    ...
    def _fees(self):
        ...
        if self.get_balance() < self._balance_threshold:
            self.add_transaction(self._low_balance_fee, date=str(new_date), exempt=True)
